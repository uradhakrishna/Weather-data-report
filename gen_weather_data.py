...
def main(argv):
    ...
    try:
        ...
        print ("generate_data.py -i (<tif file path>/<tif file name.tif>  -r (random station selection) <y/n>")
        sys.exit()
    for opt, arg in opts:
        if opt == '-h':
            print ("generate_data.py -i (<tif file path>/<tif file name.tif> -r (random station selection) <y/n>")
...
if __name__ == '__main__':
    ...
    # Read raster
    with rasterio.open('/Users/uppugr1/workspace/weather-data-master/geo.tif') as r:
        T0 = r.transform
    ...
    rc2en = lambda r, c: (c*T1[0], r*T1[1])
    ...
    #x,y  = Transformer.transform(p1, eastings, p2,northings)

    longs,lats = Transformer.transform(p1,p2,eastings, northings)


    weatherFile = open("weather_output_data.dat", "w")
    ...
    for r in range(0, len(longs)):
        ...
        for i in range(0, len(s_long)):
            ...
            datetime = randomDate("2019-1-1 00:00:00", "2020-1-1 23:00:00", random.random())
            weather = genWeather()
